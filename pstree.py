#!/usr/bin/env python
# from https://code.google.com/p/berkeley-coreference-analyser/source/browse/nlp_util/pstree.py
# 8516e1a92fb4 by Jonathan Kummerfeld <m...@jkk.name> on Oct 30, 2013
# my own edits added
# * ability to read in parse trees (ignore weights)
# * connection to head_finder
# usage:
# >>> import pstree
# >>> with open('tree') as f:
# ...     line=f.readline()
...
# ... 
# this time , the us selling weapons to taiwan is as high as more than us $ 5.8 billion , gross interference in the internal affairs of our country .

# -*- coding: utf-8 -*-
import jmutil
import head_finder as hf
...
class PSTree:
  ...
  def get_head(self, node):
    ''' Get the head word given a node '''
     # TODO: memoize the head finder
    heads=hf.collins_find_heads(self)
    key=(node[1:3], node[0])
    val=heads[key]
    return val[1]

def tree_from_text(text, allow_empty_labels=False, allow_empty_words=False, allow_weights=True):
  ...
  for char in text:
    ...
    if char == '(':
      ...
      if cur.label is DEFAULT_LABEL:
        ...
        cur.label = word
        word = ''
      if word != '' and not (allow_weights and jmutil.isFloat(word)):
